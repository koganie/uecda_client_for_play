...
void makeRestrictedAllYaku(vector<Yaku> *allYaku, const int hands[8][15]){
    //handsから作ることが可能なすべての役を作る
    //cout<<"a "<<allYaku->size()<<endl;
	makeRestrictedKaidanFrom815(allYaku, hands);//階段
    makeRestrictedPairFrom815(allYaku, hands);//ペア
    makeTankiFrom815(allYaku, hands);//単騎
    makePass(allYaku);//パス
}
...
void makeRestrictedKaidanFrom815(vector<Yaku> *yaku, const int hands[8][15]){
    //handsから作ることの可能な階段役をyakuに入れる
	//記述が品雑になるので場の状況は一切考えない
	//あとで合致するものだけ取り出す
	bool joker = (hands[4][1]!=0);
	
	int suit, rank;
	for(suit=0; suit<4; suit++){//スートごと一列ずつ見る
		for(rank=0; rank<13;rank++){//始点を決定する（あり得るのは[12-13-14]マデ）
			int length = 0;         //伸びる長さ（役の枚数）
			bool joker_flag = joker; //jokerは使えるか。
			bool searching = true;      //探索中
			while( searching ){
				if(rank+length<15 && hands[suit][rank+length]==1){//カードがあるなら
					length++;
				}else if( joker_flag ){//なくてもジョーカーが使えるなら
					length++;
					joker_flag = false;
				}else{//作れない場合はおしまい
					searching = false;
				}
				if(searching && length>=3){//階段が作れる
					int64 hd = IS_KAIDAN;
				    //hd |= (length<6) ? IS_NUM(length) : IS_NUM(6);//6?枚まではビットが使える
					int i, JposSuit=-1, JposRank=-1;
					int bitSuit = (1<<suit);
					...
					if( joker_flag ){//jokerを使わずに来ていれば、jokerに置き換えた階段も作ることができる
                        /*
						for(i=rank;i<rank+length;i++){
							int64 mask = ~CARDBIT(suit, i);//joker使うとこだけ0
						    int64 hdj = (mask&hd)|IS_JUSED;//残すものは残す
                        ...
                        */
                        //有益と思われるのは
                        //1. 強い札としてjokerを使って、例えば2を1枚温存するとか
                        //2. 8切りでjokerを使って、例えばスペ３を嫌って8だけ残すとか
					}
...
void makeRestrictedPairFrom815(vector<Yaku> *yaku, const int hands[8][15]){
	//記述が品雑になるので場の状況は一切考えない
	//あとで合致するものだけ取り出す
	...
	for(rank=1;rank<=13;rank++){//各ランクについてみていく
		for(pat=0;pat<PATTERN_NUM; pat++){//それぞれのパターンについて
			...
			if(num == match){//jokerを使わずに作ることができる
				...
				yaku->push_back(cd);
                /*
                //jokerに置き換えるバージョンに優位性が感じられない…
				if( joker ){//jokerに置き換えたバージョンを（8で使うくらいしか利点がわからない）
				    //hd |= 0ull;
					for(suit=0;suit<4;suit++){//jokerに置き換えるスート
						if(PATTERN[pat][suit]==1){
						    int64 mask = ~CARDBIT(suit, rank);//ジョーカーを使うところだけ0
