...
bool Table::sameBafuda(int bafuda[8][15]){
    for(int i=0;i<4;i++){
        for(int j=0;j<15;j++){
            ...
            */
            if( bafuda[i][j]==0 && mBafuda.getCardBit()&CARDBIT(i,j) ){
                return false;
            }else if( bafuda[i][j]==1 && !mBafuda.getCardBit()&CARDBIT(i,j) ){
                return false;
            }else if( bafuda[i][j]==2 && !mBafuda.getCardBit()&IS_JUSED ){
                return false;
            }
        }
    }
